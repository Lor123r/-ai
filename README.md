...
**当前状态**：MVP 的十个阶段全部完成，验证门禁全绿。README 第 14 章原先列的五个后续方向里，除全文搜索外都已落地。仓库已公开发布，安装包走 GitHub Release 分发。
...
| `npm run package:installer` | 先 `build` 再打成 NSIS 安装器 `release/ebook-reader-<版本>-x64.exe` |
| `npm run verify` | **交付门禁**：`typecheck` → `test` → `test:e2e` |

> 交付前一律以 `npm run verify` 为准。它失败就等于这次改动没做完。
> 三个 `package:*` 命令**不在** `verify` 里：它们要下载 Electron 二进制、耗时数分钟，
...
```jsonc
"publish": [
  { "provider": "github", "owner": "Lor123r", "repo": "-ai" }
]
```

用 `github` provider 而不是 `generic`，是因为 `latest.yml` 与安装包本来就该待在同一个 Release 里，让 electron-builder 自己算地址比手写 URL 少一处会写错的地方。应用会在这个地址后面拼上 `latest.yml`。改地址不用改代码。

> **检查更新只在公开仓库上成立。** 私有仓库的 Release 附件需要认证，而应用发的是匿名请求，拿到的会是 404。所以这个仓库是 public 的——这不是偏好，是功能前提。
...
npm run package:installer # NSIS 安装器，适合正式分发
```
...
| 书架 | 封面网格、书名、作者、进度条、删除（连带回收这本书的 epub、封面、书签与划线）；排序（最近阅读 / 导入时间 / 书名 / 作者 / 阅读进度）与筛选（全部 / 在读 / 未开始 / 已读完的 / EPUB / TXT）在书架顶部切换，默认「最近阅读 → 导入时间倒序 → id」 |
...
书架原本只有一种顺序（最近阅读 → 导入时间倒序 → id），书一多就找不到东西。排序与筛选是纯前端的重排与过滤，**不落盘、不加 IPC、不动存储**。

1. **排序与筛选是「视图偏好」，不是内容。** 落盘意味着要新增一份存储，或把书架偏好塞进语义是「阅读设置」的 `settings.json`（字号/主题/字体），后者会让 `ReaderSettings` 这个名字开始说谎。收益只是「重启后还记得上次选的排序」，而用户打开应用第一件事是找书，不是确认下拉框还停在「按书名」。零存储 = 零 IPC = 零迁移 = 零损坏路径，这一轮只碰渲染层。代价是重启后回到默认（最近阅读 / 全部），已写进第 14 章的当前限制。
2. **`ShelfEntry` 从渲染层的 hook 搬进 core。** 排序与筛选是纯函数、属于领域逻辑，而 core 不能反过来依赖 renderer。所以 `ShelfEntry` 定义在 [shelfView.ts](src/core/domain/shelfView.ts)，[useBooks.ts](src/renderer/src/hooks/useBooks.ts) 改成再导出，既有 import 路径一行不用改。
3. **中文书名走 `localeCompare` 而不是 `<`。** 按 `<` 排是按 UTF-16 码元排，「三体」（`三` U+4E09）会排在「活着」（`活` U+6D3B）前面，但用户期望的是拼音序。`Intl.Collator('zh-Hans-CN')` 在 Node 与 Chromium 里都走 ICU。代价是 ICU 版本不同结果可能微差，所以测试只用「明显该在前/在后」的样本，不钉死具体顺序。
4. **无作者的书排最后。** `author ?? ''` 会让空串在升序里排最前，而「未知作者」堆在书架最前面是最没用的排法，所以显式把 `null` 排到最后。
5. **所有排序都以 id 兜底。** 与 `compareBooksForShelf` 同一条约定：保证任何实现、任何输入顺序下结果完全一致，测试才能钉死。
6. **`reading` / `unread` / `finished` 三者互斥且穷尽「全部」。** `reading` 是「开了头没读完」，`unread` 是「一次没开过」，两者加起来就是未读完。这样用户不会问「我读了一半的书去哪了」。`finished` 复用 `progress.ts` 的 `isBookFinished`（阈值 99.5%），不另写一份。
7. **筛选后为空要给专门的空态。** 复用「书架还是空的，导入 EPUB 或 TXT 后就会出现在这里。」是假话——书架上有书，只是不符合筛选。所以另给一句「没有符合条件的书，换个筛选试试。」，并且**工具条必须留着**，否则用户没法切回「全部」。
8. **计数在筛选时显示「筛出 / 总数」。** 筛出 3 本却看到「12 本」会让人以为筛选坏了。
9. **用原生 `<select>` 而不是一排按钮。** 排序 5 项、筛选 6 项，按钮排会占掉一整行且随选项增加而变宽；原生控件还自带键盘操作与无障碍语义。
10. **筛选选项的文案要避开卡片上已有的文案。** 「已读完」既是筛选选项也是卡片上的进度文案，两处同名会让 `getByText` 撞车（测试先撞上了）。筛选选项改成「已读完的」，读起来也更像在描述一个集合。
...
7. **Windows 分发做三种形态：免安装目录（`dir`）、压缩包（`zip`）与 NSIS 安装器（`nsis`）。** 前两者适合本机验证与拷给别人，安装器适合正式分发。三者共用同一份 `files` 与 `artifactName`，只是打包方式不同。
8. **安装器装到用户目录（`%LOCALAPPDATA%\Programs\电纸书阅读器`），免管理员权限。** `nsis.perMachine: false`。装进 `Program Files` 要 UAC 提权，而一个本地阅读器没有任何需要提权的理由；提权还会让「双击安装」变成一次系统级操作，用户会犹豫。
9. **安装器走向导（`oneClick: false`）并允许改安装目录（`allowToChangeInstallationDirectory: true`）。** 一键安装会把应用直接塞进默认目录，用户连装到哪都看不到。走向导多两次点击，换来的是「我知道它装哪了」。
10. **卸载时保留用户数据（`deleteAppDataOnUninstall: false`）。** 书库与批注是用户资产，卸载应用不该顺手删掉。用户想彻底清干净，自己去删 `%APPDATA%\电纸书阅读器` 即可——**删数据应该是显式动作，不是卸载的副作用**。
11. **不做开机自启。** 阅读器不是常驻工具，没有理由占着开机启动项。`nsis.runAfterFinish`（装完立刻启动应用）保留默认 `true`，它与开机自启无关。
12. **不做代码签名与自动更新。** 签名需要真实证书与密码，仓库里放不了；自动更新需要发布服务器 + 签名，且会引入一个常驻网络请求。未签名的安装器首次运行会被 SmartScreen 拦一下，这是已知代价。
13. **`files` 只收 `out/**/*` 与 `package.json`。** 默认规则会把整个仓库塞进 asar，`src/`、`tests/`、`e2e/` 全进去。这条由 [packaging.test.ts](tests/unit/repo/packaging.test.ts) 钉住。
14. **`RuntimeVersions` 加 `app` 字段，标签去掉 Chromium 与 Node。** 用户报 bug 时关心的是「哪个版本的应用」，Chromium 版本是开发者信息且随 Electron 版本唯一确定。用 `app.getVersion()` 而不是 `process.env.npm_package_version`——打包后没有 npm 环境，这个变量是空的。
15. **⭐ 应用版本必须走 IPC 拿，不能在 preload 里读 `app`。** `app` 是**主进程专属**模块，preload 跑在渲染进程里，`electron.app` 是 `undefined`。`app.getVersion()` 会在 preload 加载时直接抛错，把整个 `contextBridge.exposeInMainWorld` 一起带走——表现是 `window.api` 变成 `undefined`，界面上所有功能静默失效，而主进程侧看不到任何报错。所以新增了 `runtime:versions` 频道（[runtimeIpc.ts](src/main/ipc/runtimeIpc.ts)），`AppBridge.versions` 相应改成 `Promise<RuntimeVersions>`，渲染层用 [useRuntimeVersions.ts](src/renderer/src/platform/useRuntimeVersions.ts) 在 effect 里取。`process.versions.*` 在 preload 里本来可用，但为了「一次调用拿全」还是并到主进程一起返回。
16. **⭐ 探针的默认值必须来自运行时，且必须真机验证一次。** 这一轮踩的坑：`defaultProbe()` 里写死了 `isPackaged: false`，从来没读 `app.isPackaged`，于是 `resolvePortableDataDir` 的第一道闸在生产环境永远命中，便携模式**完全没生效**。而 16 条单测全绿——因为测试自己注入探针，**绕过了这个写死的默认值**。修法是把 `isPackaged` 提升成必填参数（`defaultProbe(isPackaged)`、`applyUserDataOverride(env, isPackaged, setPath, probe?)`），漏传直接编译报错。教训：**凡是「默认值来自运行时环境」的探针，都必须有一次真机验证，否则单测的绿灯是假的。**
...
启动时问一次更新源，有新版本就在书架顶部提示。整条链路是「读 `latest.yml` → 比版本号 → 出文案」，**没有下载、没有安装、没有常驻轮询**。

1. **只做「检查 + 提示」，不做下载与安装。** 未签名的更新包在 Windows 上会被 `electron-updater` 拒绝（它要校验发布者签名），所以自动安装这条路在拿到代码签名证书之前走不通。与其摆一个点了必然失败的「立即更新」按钮，不如老实告诉用户「有新版本，去下载」。`UpdateBridge` 因此**只有一个 `check()`**，刻意不摆 `download()` / `install()` 空壳——空壳会让人以为这条路已经通了。
2. **版本比较放 core 而不是主进程。** 它是纯函数、无 IO，且最容易写错（`1.10.0` 与 `1.9.0` 按字典序比会得出反的结论）。放 [update.ts](src/core/domain/update.ts) 才能被单测直接覆盖，也让主进程那层只剩「取文本 + 解析」。
3. **`parseVersion` 刻意不支持预发布后缀**（`1.0.0-beta.1` 判为解析失败）。把预发布语义做对需要一整套比较规则（`beta.1` < `beta.2` < `rc.1` < 正式版），而本项目根本不发预发布版，收益为零；反过来把 beta 当成「比正式版小」会让用户永远收不到提示。
4. **`compareVersions` 任一侧解析失败返回 0（当作一样）。** 读不懂就不提示升级——读不懂的版本号很可能来自格式变更，提示升级会把用户引到一个没验证过的状态。
5. **`evaluateUpdate` 在 `currentVersion` 解析失败时返回 `malformed` 而不是 `up-to-date`。** 应用自己的版本号读不出来说明构建有问题，这时候说「已是最新」是在撒谎。
6. **`UpdateUnavailableReason` 分四档**（`not-packaged` / `no-feed` / `network` / `malformed`），因为对用户的意义完全不同：前两者是「这个构建本来就不该检查」，界面上一个字都不该显示；后两者是「检查失败了」，但重试有用没用不同。合成一个 `failed` 会让界面只能说含糊的「检查更新失败」。
...
10. **更新源地址写在 `package.json` 的 `build.publish` 里，运行时从 `resources/app-update.yml` 读。** 这样「发布到哪」是打包配置的一部分：改地址不用改代码，也不会出现「代码里写死一个地址、打包配置里写另一个」的分裂。用 `github` provider 而不是 `generic`，是因为 `latest.yml` 与安装包本来就该待在同一个 Release 里，让 electron-builder 自己算地址比手写 URL 少一处会写错的地方。**读不到 `app-update.yml` 就是「这个构建不检查更新」**——开发态、以及没配 `build.publish` 的构建都会走到这里。应用刻意不去猜这个地址：猜错的表现是静默地永远检查不到更新，比不检查更糟。
...
| `runtime` | `runtime:versions` | — | `RuntimeVersions` |
| `update` | `update:check` | — | `UpdateCheckResult` |
...
### 单元测试地图（80 文件 / 1175 用例）

| 分组 | 文件数 | 用例数 | 关注点 |
| --- | --- | --- | --- |
| `core/domain` | 11 | 238 | 归一化、复活、排序、进度换算、目录摊平与目标解析、书签划线的收敛与拒绝、TXT 分块与块内页反解（含按块内偏移换算页码）、按标题行生成 TXT 目录（含罗马数字、后置序号、英文标题与纯编号行）、书架的排序与筛选（含中文拼音序、无作者排最后、三种进度筛选互斥且穷尽）、版本号解析与比较（含 `1.10.0` 大于 `1.9.0` 这类字典序陷阱、预发布后缀判为解析失败、读不懂就不提示升级） |
...
| `tests/unit/repo` | 4 | 45 | `.claude/agents` 子 Agent 定义：命名、frontmatter 完整、在 `AGENTS.md` 里被引用、无命令执行能力；注解与书库存储层的源码级隔离；`tools/make-fixtures.ps1` 的 BOM 与语法、以及它生成的复核样本（覆盖 R21–R29、字节数与实际文件对账、块数、行尾归一化、三本长文字节互异）；打包配置（`main` 指向 `out/main/index.js`、`files` 只收 `out/**/*` 与 `package.json`、产物目录已 gitignore、只打 `dir` / `zip` / `nsis`、不用 `portable` target、`artifactName` 与 `appId` 是 ASCII、打包脚本先 build 且不进 verify、electron-builder 是 devDependency、NSIS 走向导且装用户目录、允许改安装目录、卸载保留数据、建桌面与开始菜单快捷方式、更新源是 `github` provider 且 owner/repo 指向真实仓库、不残留 `example.com` 占位地址） |
...
- 更新**只检查、只提示，不下载也不安装**（见第 7.13 章）：未签名的更新包在 Windows 上会被 `electron-updater` 拒绝，所以拿到代码签名证书之前自动安装这条路走不通。更新源地址写在 `package.json` 的 `build.publish` 里（`github` provider，打包时写进 `resources/app-update.yml`），指向本仓库的 Release。**检查更新只在公开仓库上成立**：私有仓库的 Release 附件需要认证，而应用发的是匿名请求。另外 `app-update.yml` 只有 `nsis` target 会生成，所以免安装目录版与 zip 版不检查更新。
...
- 书架的排序与筛选是**会话内**的视图偏好，不落盘：重启后回到默认（最近阅读 / 全部）。理由见第 7.11 章。
- 书架没有分组与标签：按作者/格式分节与筛选高度重叠，自定义标签需要独立一轮（新存储 + 新 IPC + 打标签入口）。

### 后续方向

第 14 章原先列的五个方向（TXT 目录质量、书签划线细节、全文搜索、书库组织、打包分发）里，**除全文搜索外都已落地**。下面按「还剩什么」重列。
