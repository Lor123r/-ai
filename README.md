...
| 29 | — | 修复 | 清掉三处挂账：删书的收尾失败按性质分级留痕（文件被占用降为告警、越界与未知错误升为错误）；`registerBooksIpc` 收成 deps 对象与 `registerLibraryIpc` 对齐；导入写入书库失败时回收刚复制进来的 epub 与封面，并让单个文件失败不再中断整批；`JsonBookRepository` 四条写操作落盘失败时回滚内存 |
