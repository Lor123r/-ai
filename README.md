...
| 36 | `—` | 修复 | 书卡的开书热区只有书名那一小块文字，封面这块面积最大的地方是死区 —— 实测书名按钮 45x21、封面 162x216，点封面时整卡唯一的 `onClick` 根本没被触发，界面上看起来就是「点了没反应」。改法是把 `.book-card` 设为定位基准，书名按钮用 `::after { inset: 0 }` 把热区撑满整张卡片（仍然只有一个交互元素，不必把封面里的 `img` 改成按钮），删除按钮抬到覆盖层之上免得被吃掉；端到端用例改为按坐标点封面进书，并补上书名按钮仍可点的断言 |
